...
public class SpiderMachine extends Boss {
    ...
    public SpiderMachine(FightManager fightMNG, byte idGun, String name, byte location, int HPMax, short X, short Y)
            throws IOException {
    ...
    public void turnAction() {
        try {
            ...
            if (Math.abs(X - pl.X) <= 40 && Math.abs(Y - pl.Y) <= 30) {
                ...
                this.fightMNG.newShoot(this.index, (byte) 8, (short) Until.getArgXY(X, Y, pl.X, pl.Y), (byte) 30,
                        (byte) 0, (byte) 1, false);
            ...
            switch (Until.nextInt(3)) {
                ...
                case 0:
                    this.itemUsed = 9;
                    FA = fightMNG.getForceArgXY(idNV, this.fightMNG.bulletManager, false, X, Y, pl.X,
                            (short) (pl.Y - (pl.height / 2)), (short) (pl.width / 2), pl.height, 50, 5, 70, 70);
                    if (FA == null) {
                ...
                case 1:
                    this.itemUsed = 16;
                    FA = fightMNG.getForceArgXY(idNV, this.fightMNG.bulletManager, false, X, Y, pl.X,
                            (short) (pl.Y - (pl.height / 2)), (short) (pl.width / 2), pl.height, 50, 5, 10, 50);
                    if (FA == null) {
                        if (!fightMNG.checkWin())
                            fightMNG.nextTurn();
                        return;
                ...
                // rôcet
                case 2:
                    FA = fightMNG.getForceArgXY(idNV, this.fightMNG.bulletManager, true, X, Y, pl.X,
                            (short) (pl.Y - (pl.height / 2)), (short) (pl.width / 2), pl.height, 50, 5, 30, 50);
                    if (FA == null) {
                        if (!fightMNG.checkWin())
                            fightMNG.nextTurn();
                        return;
