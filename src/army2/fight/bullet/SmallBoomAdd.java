...
public class SmallBoomAdd extends Bullet {
    ...
    public SmallBoomAdd(BulletManager bulletManager, byte bullId, int satThuong, Player pl, int X, int Y, int vx,
    ...
    public void nextXY() {
        ...
        if (super.collect && X > 0 && X < fm.mapMNG.Width && Y < fm.mapMNG.Height) {
            Player players;
            try {
                players = new SmallBoom(fm, (byte) 11, "Small Boom", (byte) fm.allCount, 1000 + (fm.getLevelTeam() * 8),
                        X, Y);
